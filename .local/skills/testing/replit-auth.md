...
Replit Auth allows for testing arbitary users (assuming the application uses the standard OIDC Replit Auth implementation). Make sure you mention in the testing task that you are testing a Replit Auth application and make it abundantly explict that login claims need to be overridden.
...
After testing, the OIDC ISSUER override might still be active. Restart the API server workflows to make sure the override is cleared, if needed.
