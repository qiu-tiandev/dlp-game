...
For Databricks Apps, use this database normally during development. Replit exposes it through the same database tools and PostgreSQL environment variables as other projects. When the user publishes, Replit natively provisions or reuses a production Lakebase database in Databricks and binds it to the deployed app. Do not create a separate Lakebase instance.

### Database targets and permissions

Lakehouse analytical tables are queried through a Databricks SQL warehouse. Lakebase is the PostgreSQL database for app-owned transactional data; it is not the SQL warehouse.

For agent queries through the `database` skill's `executeSql`, choose the target explicitly:

| Task | Tool arguments | Agent access |
| --- | --- | --- |
| Create schema, seed data, or test CRUD in development | `target: "replit_database", environment: "development"` | Read/write, subject to normal database safety checks |
| Inspect production Lakebase data | `target: "replit_database", environment: "production"` | Read-only through the agent's production role |
| Query lakehouse analytical data through the warehouse | `target: "databricks"` (omit `environment`) | Warehouse access through the Databricks connector, not Lakebase |

The agent's production role is read-only; this does not make the deployed app read-only. Build and test application reads and writes against the development database using the existing `lib/db` connection and `DATABASE_URL`. Publish binds the deployed app to its managed production Lakebase connection for user-driven application reads and writes.

Do not obtain production write credentials or use scripts, application endpoints, or other connections to bypass the agent's read-only production access. If asked to edit production records directly, explain that the user must make those edits. Continue implementing and testing requested CRUD functionality in development. Production schema changes go through Publish as described in the `database` skill.
